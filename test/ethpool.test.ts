...
describe('ETHPool', () => {
  ...
  describe('deposit', async () => {
    it('should be deposited', async () => {
      await ethPool.connect(signers[0]).depositEth({ value: 100 })
      const currentUser0Balance = await ethPool.connect(signers[0]).getMyEthBalance()
      ...
      const currentUser1Balance = await ethPool.connect(signers[1]).getMyEthBalance()
